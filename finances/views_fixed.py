...
def transactions(request):
    ...
    income_categories = categories.filter(type='income').order_by('order', 'name')
    expense_categories = categories.filter(type='expense').order_by('order', 'name')
    ...
    context = {
        'transactions': transactions,
        'categories': categories,  # Keep all categories for backward compatibility
        'income_categories': income_categories,  # Add income categories separately
        'expense_categories': expense_categories,  # Add expense categories separately
...
def create_transaction_api(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Debug category info
            debug_category_icon = request.POST.get('debug_category_icon', '')
            debug_category_text = request.POST.get('debug_category_text', '')
            print(f"Debug Category Info - Icon: {debug_category_icon}, Text: {debug_category_text}")
            
            # Debug: Print all parameters
            print(f"Title: {title}, Amount: {amount}, Date: {date_str}, Time: {time_str}, Type: {transaction_type}")
            print(f"Category ID: {category_id}, Subcategory ID: {subcategory_id}, Account ID: {transaction_account_id}")
            print(f"Currency: {currency}, Notes: {notes}")
            ...
            if not title or not amount or not date_str or not transaction_type:
                ...
                if not transaction_type: missing_fields.append("type")
            ...
            if category_id and category_id != 'null' and category_id != '':
                try:
                    ...
                    # Ensure the category has an icon
                    if not category.icon and debug_category_icon:
                        print(f"Setting category icon from frontend: {debug_category_icon}")
                        category.icon = debug_category_icon
                        category.save()
                except Exception as e:
                    print(f"Error getting category: {e}")
                    # Don't return error - category is optional
            ...
            if subcategory_id and subcategory_id != 'null' and subcategory_id != '':
                try:
                    ...
                    # Ensure the subcategory has an icon if category doesn't
                    if category and not subcategory.icon and debug_category_icon:
                        print(f"Setting subcategory icon from frontend: {debug_category_icon}")
                        subcategory.icon = debug_category_icon
                        subcategory.save()
                except Exception as e:
                    print(f"Error getting subcategory: {e}")
                    # Don't return error - subcategory is optional
            
            # Set transaction account if provided
            if transaction_account_id and transaction_account_id != 'null' and transaction_account_id != '':
                try:
                    ...
                    # Don't return error - account is optional
            ...
            print(f"Transaction created successfully with ID: {transaction.id}")
                ...
                # Include category info in the response
                return JsonResponse({
                    'success': True, 
                ...
                detailed_error = f"Error saving transaction: {str(save_error)}"
                print(detailed_error)
                print(f"Transaction data: {transaction.__dict__}")
                return JsonResponse({'success': False, 'error': detailed_error})
            
        except Exception as e:
            detailed_error = f"Error creating transaction: {str(e)}"
            print(detailed_error)
            import traceback
        ...
        try:
            ...
            if time_str:
                try:
                    ...
                    print(f"Error setting time: {e}")
                    # Keep existing time if there's an error
            ...
            detailed_error = f"Error updating transaction: {str(e)}"
            print(detailed_error)
            import traceback
            print(traceback.format_exc())
            return JsonResponse({'success': False, 'error': detailed_error})
